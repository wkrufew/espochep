        ...
        <div class="overflow-x-auto sm:mx-0.5 lg:mx-0.5">
          <div class="py-2 inline-block min-w-full sm:px-6 lg:px-8">
            <div class="pb-4 flex">
                ...
                <a class="px-4 py-1 text-sm bg-blue-600 ml-4 text-white rounded-full flex justify-center items-center" href="{{ route('moderador.transparencies.create') }}">Nuevo Año</a>
            </div>
            ...
            <div class="overflow-hidden">
              <table class="min-w-full rounded-lg table-auto overflow-hidden">
                ...
                <tbody class="rounded-full">
                    @forelse ($transparencies as $key => $transparency)
                        <tr class="bg-white border-b transition duration-300 ease-in-out hover:bg-gray-100">
                            ...
                            <td class="text-xs text-gray-900 font-semibold px-6 py-4 text-center">
                                {{$transparency->created_at->isoFormat('dddd, D MMMM, Y')}}
                            </td>
                            <td class="text-xs text-white h-full text-center">
                                @if ($transparency->status == 1)
                                    <span class="rounded-full py-1 px-3 bg-red-500">Borrador</span>
                                @elseif($transparency->status == 2)
                                    <span class="rounded-full py-1 px-3 bg-green-500">Publicado</span>
                                @endif
                            </td>
                            <td class="text-base text-center text-gray-900">
                                    <a title="Editar/Ver" href="{{ route('moderador.transparencies.edit', $transparency) }}" class="rounded-full text-blue-600 px-2 py-2"><i class="fa-solid fa-pen-to-square"></i></a>
                                    {{-- <a title="Elimnar" href="{{ route('moderador.transparencies.eliminar', $transparency) }}"><i class="fa-solid fa-trash-can text-red-500"></i></a>
                                     --}}
                            </td>
                        </tr>
                    @empty
