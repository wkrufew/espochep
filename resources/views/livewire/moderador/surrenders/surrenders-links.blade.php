<div>
    @foreach ($stage->links as $item)
    
    <article class="card bg-white border border-blue-400 rounded-lg py-2 mt-4 mx-6" x-data="{open: false}">
        <div>

            @if ($link->id == $item->id)
                <form wire:submit.prevent="update" class="px-3">
                    <div class="flex-col items-center">
                        <label class="w-32 text-xs font-semibold">Titulo: </label>
                        <input wire:model.lazy="link.title"  class="block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-neutral-300 focus:border-neutral-300 sm:text-sm">
                    </div>
                    @error('link.title')   
                    ...
                    <div class="flex-col items-center">
                        ...
                        <input wire:model.lazy="link.url_file"  class="block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-neutral-300 focus:border-neutral-300 sm:text-sm">
                    </div>
                    <!--BOTONES PARA EDITAR Y ELIMINAR UNA LECCION-->
                    <div class="flex justify-center mt-4">
                        <button type="button" class="rounded-full px-3 py-1 text-white bg-neutral-800 text-xs" wire:click="cancel">Cancelar</button>
                        <button type="submit" class="rounded-full px-3 py-1 text-white bg-blue-600 text-xs ml-4">Actualizar</button>
                    </div>
                </form>
            @else
                <header>
                    <h1 x-on:click="open = !open" class="cursor-pointer text-sm ml-6"><span class="font-semibold pr-1">Titulo: </span> {{$item->title}} </h1>
                    <h1 x-on:click="open = !open" class="cursor-pointer text-sm ml-6 pt-1"><span class="font-semibold pr-1">Link: </span> {{$item->url_file}} </h1>
                </header>
                <div x-show=open>
                    <hr class="my-2">
            ...
            <div class="card-body bg-white">
                ...
                <div class="space-y-1">
                    <div class="flex items-center">
                        ...
                        <input wire:model.defer="title"  class="block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    </div>
                    @error('title')   
                    ...
                    <div class="flex items-center">
                        ...
                        <input wire:model.defer="url_file"  class="block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    </div>
                    @error('url_file')   
                        <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-1 text-sm rounded relative mt-1" role="alert">
                            <strong class="font-bold">Ups!</strong>
