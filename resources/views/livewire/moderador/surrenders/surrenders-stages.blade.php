<div>
    ...
    @foreach ($surrender->stages as $item)
        <article class="mb-6" x-data="{open: false}">
            <div class="bg-gray-50 border border-neutral-300 p-3 rounded-lg">
                @if ($stage->id == $item->id)
                    <form wire:submit.prevent = "update" class="space-y-2">
                        <input wire:model="stage.name" class="block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" placeholder="Ingrese una fase...">
                        @error('stage.name')   
                ...
                @else
                    <header class="flex justify-between items-center ">
                        <div x-on:click="open = !open" class="text-sm space-y-1">
                            <h1 class="cursor-pointer text-neutral-800 font-semibold"> {{$item->name}}</h1>
                        </div>
                        <div>
                    ...
                    <div x-show="open">
                        @livewire('moderador.surrenders.surrenders-links', ['stage' => $item], key($item->id))
                    </div>
    ...
    <div x-data = "{ open : false}">
        ...
        <article x-show="open" class="bg-gray-100 mt-3 p-3 rounded-lg">
            <div class="card-body">
                <h1 class="text-base font-bold text-blue-500">Nueva Fase</h1>
                <div>
                    <input wire:model="name" class="mt-2 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="Ingrese una fase valida">
                    @error('name')   
