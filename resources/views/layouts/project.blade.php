    ...
    <div class="min-h-screen bg-gray-100">
                        ...
                        <div class="flex space-x-4">
                            ...
                            <div class="w-full my-auto">
                                ...
                                {{-- <form action="{{ route('localization') }}" id="formLocalization">
                                    <select onchange="document.getElementById('formLocalization').submit();"
                                        name="lang"
                                        class="form-select appearance-none hover:text-neutral-500 inline-flex items-center pr-10 pl-4 py-1.5 border border-transparent text-sm font-medium leading-4 rounded-full text-white bg-neutral-700">
                                        <option class="py-4 px-3" value="es"
                                            @selected(session('lang') == 'es')>{{ __('Español') }}</option>
                                        <option class="py-4 px-3" value="en" @selected(session('lang') == 'en')>
                                            {{ __('Ingles') }}</option>
                                    </select>
                                </form> --}}
                            </div>
        ...
        <!-- CONTENIDO PRINCIPAL DEL A PLANTILLA DE MODERADORES -->
            <div class="max-w-7xl mx-auto rounded-lg px-6 py-4 grid grid-cols-1 lg:grid-cols-5 lg:gap-4">
                <aside class="mb-4">
                    <h1 class="font-bold mx-2 py-2 rounded-lg text-sm mt-2  mb-4 text-center bg-neutral-900 text-white">DETALLES DEl PROYECTO</h1>
                    <ul class="overflow-hidden bg-white text-sm p-3 rounded-lg shadow-lg text-neutral-900 mb-10 ml-2 mr-2">
                        <li class="my-1 leading-7 rounded-lg pl-2 @routeIs('moderador.projects.edit', $project) bg-neutral-900 text-white @else  @endif hover:bg-red-700 hover:text-neutral-50">
                            <a href="{{ route('moderador.projects.edit', $project) }}">Informacion del Proyecto</a>
                        </li>
                        <hr>
                        <li class="my-1 leading-7 rounded-lg pl-2 @routeIs('moderador.projects.curriculum', $project) bg-neutral-900 text-white @else @endif hover:bg-red-700 hover:text-neutral-50">
                            <a href="{{ route('moderador.projects.curriculum', $project) }}">Etapas del Proyecto</a>
                        </li>
                        <hr>
                        <li class="my-1 leading-7 rounded-lg pl-2 hover:bg-red-700 hover:text-neutral-50 ">
                            <a href="{{ route('moderador.projects.index') }}">Listado de Proyectos</a>
                        </li>
                    </ul>
                        ...
                        @case(1)
                            <div class="text-center bg-white p-4 rounded-lg">
                                <div class="bg-red-500 text-gray-50 rounded-lg">
                                    <p class="text-sm text-center px-3 py-1">Proyecto en Borrador</p>
                                </div>
                            </div>
                            @break
                        @case(2)
                            <div class="text-center bg-white p-4 rounded-lg">
                                <div class="bg-green-600 text-gray-50 rounded-lg">
                                    <p class="text-sm text-center px-3 py-1">Proyecto en Ejecucion</p>
                                </div>
                            </div>
                            @break
                        @case(3)
                            <div class="text-center bg-white p-4 rounded-lg">
                                <div class="bg-neutral-700 text-gray-50 rounded-lg">
                                    <p class="text-sm text-center px-3 py-1">Proyecto Terminado</p>
                                </div>
                            </div>
                            @break      
                        @default
